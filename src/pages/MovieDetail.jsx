...
const MovieDetail = () => {
  ...
  useEffect(() => {
    if (movie) {
      ...
      const storedReviews = JSON.parse(localStorage.getItem(`reviews_${movie.id}`) || '[]');
  ...
  const handleSubmitReview = (e) => {
    ...
    if (!hasPurchasedTicket()) {
      alert('Bạn cần đặt vé và thanh toán cho bộ phim này trước khi đánh giá');
      return;
    }

    if (hasReviewed()) {
      alert('Bạn đã đánh giá bộ phim này rồi. Mỗi tài khoản chỉ được đánh giá một lần.');
      return;
    }
        ...
        <div className="col-md-4 mb-4">
          ...
          <div className="mt-3">
            {movie.status === 'now-showing' ? (
              <Link to={`/book/${movie.id}`} className="btn btn-success w-100 btn-lg">
                <FaTicketAlt className="me-2" />
                Đặt vé ngay
              </Link>
            ) : (
              <button className="btn btn-warning w-100 btn-lg" disabled>
          ...
          <div className="card mb-4">
            <div className="card-body">
              ...
              <ul className="list-unstyled">
                ...
                <li className="mb-2">
                  <strong>Giá vé:</strong> <span className="text-success fw-bold">{movie.price.toLocaleString('vi-VN')}đ</span>
                </li>
              </ul>
            </div>
          </div>
            ...
            <div className="card-body">
              ...
              {user ? (
                <form onSubmit={handleSubmitReview} className="mb-4">
                  <div className="mb-3">
                    <label className="form-label fw-bold">Chấm điểm:</label>
                ...
                <div className="alert alert-info mb-4">
                  <FaInfoCircle className="me-2" />
                  Vui lòng <Link to="/auth">đăng nhập</Link> để đánh giá phim
                </div>
              )}
